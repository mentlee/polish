...
def create_tag(session: Session, name: str, tag_type_names: List[str], description: Optional[str] = None) -> Tuple[Optional[TagModel], str]:
  existing = get_tag_by_name(session, name)
  if existing:
    return [existing, "found"]

  tag_types = session.query(TagTypeModel).filter(TagTypeModel.name.in_(tag_type_names)).all()
