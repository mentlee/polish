...
def create_tag_type(session: Session, name: str, description: Optional[str] = None) -> Tuple[TagTypeModel, str]:
...
def update_tag_type(session: Session, name: str, description: str) -> Tuple[Optional[TagTypeModel], str]:
  existing = get_tag_type_by_name(session, name)
  if not existing:
    return [None, "not_found"]
