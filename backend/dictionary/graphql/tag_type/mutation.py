...
from dictionary.graphql import tag_type

from dictionary.services.tag_type import create_tag_type,remove_tag_type, update_tag_type
...
class TagTypeMutation:
  ...
  def add(input: TagTypeInput, info: Info) -> TagTypeMutationReturn:
    ...
    [tag_type, status] = create_tag_type(session, name=input.name, description=input.description)
    return TagTypeMutationReturn(tag_type=TagType.from_orm(tag_type), status=MutationStatus[status])
